...
internal abstract class BaseArchitectLevelManager(string diskFolderName)
{
    ...
    protected readonly string diskFolder = Path.Join(
        Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location),
        $"Architect-{diskFolderName}"
    );
    ...
    protected static ArchitectLevelsMetadata HashEmbedded()
    {
        ...
        {
            if (!name.ConsumePrefix(EMBEDDED_PREFIX, out var suffix))
                continue;
            if (!suffix.ConsumeSuffix(ARCHITECT_SUFFIX, out var path))
                continue;
            if (!path.Split2('.', out var groupSpan, out var sceneSpan))
                continue;
    ...
    protected string DiskLevelPath(string groupId, string sceneName) =>
        Path.Join(DiskGroupPath(groupId), $"{sceneName}{ARCHITECT_SUFFIX}");

    protected static Stream EmbeddedLevelStream(string groupId, string sceneName) =>
        Assembly
            .GetExecutingAssembly()
            .GetManifestResourceStream($"{EMBEDDED_PREFIX}{groupId}.{sceneName}{ARCHITECT_SUFFIX}");
}
