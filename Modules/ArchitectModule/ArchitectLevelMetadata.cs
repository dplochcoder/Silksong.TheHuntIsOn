...
internal class ArchitectLevelMetadata
    : Dictionary<string, SHA1Hash>,
        INetworkedCloneable<ArchitectLevelMetadata>
{
    public ArchitectLevelMetadata() { }

    public ArchitectLevelMetadata(IReadOnlyDictionary<string, SHA1Hash> dict)
        : base(dict) { }
    ...
    public void Diff(
        ArchitectLevelMetadata target,
        Action<string, SHA1Hash> updateLevel,
        Action<string> deleteLevel
    ) =>
        CollectionUtil.Compare(
            this,
            target,
            (sceneName, myHash, targetHash) =>
            {
                if (targetHash != null)
                {
                    if (myHash == null || !myHash.Equals(targetHash))
                        updateLevel(sceneName, targetHash);
                }
                else if (myHash != null)
                    deleteLevel(sceneName);
            }
        );
}
