...
internal class ArchitectLevelsMetadata
    : Dictionary<string, ArchitectLevelMetadata>,
        INetworkedCloneable<ArchitectLevelsMetadata>,
        IIdentifiedPacket<ClientPacketId>
{
    public ArchitectLevelsMetadata() { }

    public ArchitectLevelsMetadata(IReadOnlyDictionary<string, ArchitectLevelMetadata> dict)
        : base(dict) { }

    ClientPacketId IIdentifiedPacket<ClientPacketId>.Identifier =>
        ClientPacketId.ArchitectLevelsMetadata;
    ...
    public bool Contains(string groupId, string sceneName) =>
        TryGetValue(groupId, out var metadata) && metadata.ContainsKey(sceneName);

    public bool TryGet(string groupId, string sceneName, out SHA1Hash hash)
    {
        if (TryGetValue(groupId, out var metadata))
            return metadata.TryGetValue(sceneName, out hash);
    ...
    public void Diff(
        ArchitectLevelsMetadata target,
        Action<string> deleteGroup,
        Action<string, string, SHA1Hash> updateLevel,
        Action<string, string> deleteLevel
    ) =>
        CollectionUtil.Compare(
            this,
            target,
            (groupId, myGroup, targetGroup) =>
            {
                if (targetGroup != null)
                    (myGroup ?? []).Diff(
                        targetGroup,
                        (sceneName, hash) => updateLevel(groupId, sceneName, hash),
                        sceneName => deleteLevel(groupId, sceneName)
                    );
