...
internal class PauseTimerCommand : IServerCommand
{
    ...
    internal void Broadcast<T>(T packet)
        where T : IIdentifiedPacket<ClientPacketId>, new() => serverAddon.Broadcast(packet);

    private static readonly SubcommandRegister<PauseTimerCommand> subcommands = new(
        "/pt",
        [
            new PauseSubcommand(),
...
internal class PauseSubcommand : Subcommand<PauseTimerCommand>
{
    ...
    {
        if (!MaxArguments(commandSender, arguments, 1))
            return false;
        ...
        {
            if (!ParseInt(commandSender, arguments[0], out seconds))
                return false;
...
internal class CountdownSubcommand : Subcommand<PauseTimerCommand>
{
    ...
    {
        if (!MinArguments(commandSender, arguments, 1))
            return false;
        if (!ParseInt(commandSender, arguments[0], out var seconds))
            return false;
