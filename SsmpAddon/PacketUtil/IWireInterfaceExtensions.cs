...
internal static class IWireInterfaceExtensions
{
    ...
    internal static void ReadData(this ref int? self, IPacket packet) =>
        self = packet.ReadBool() ? packet.ReadInt() : null;
    ...
    internal static void ReadData(this ref long? self, IPacket packet) =>
        self = packet.ReadBool() ? packet.ReadLong() : null;
    ...
    internal static T ReadEnum<T>(this IPacket self)
        where T : Enum => (T)Enum.ToObject(typeof(T), self.ReadVarint());

    internal static void WriteData<T>(this T self, IPacket packet)
        where T : Enum => packet.WriteVarint(Convert.ToInt32(self));

    private static T ReadIntoNew<T>(this IPacket self)
        where T : IWireInterface, new()
    {
        T item = new();
        item.ReadData(self);
        return item;
    ...
    private static void Write<T>(this IPacket self, T value)
        where T : IWireInterface => value.WriteData(self);

    internal static T ReadDynamic<E, T, F>(this IPacket self)
        where E : Enum
    ...
    internal static T? ReadOptionalDynamic<E, T, F>(this IPacket self)
        where E : Enum
        where T : class, IDynamicValue<E, T, F>
        where F : IDynamicValueFactory<E, T, F>, new() =>
        self.ReadBool() ? self.ReadDynamic<E, T, F>() : null;

    internal static void WriteDynamic<E, T, F>(this IDynamicValue<E, T, F> self, IPacket packet)
        where E : Enum
        where T : IDynamicValue<E, T, F>
        where F : IDynamicValueFactory<E, T, F>, new()
    {
    ...
    internal static void WriteOptionalDynamic<E, T, F>(
        this IDynamicValue<E, T, F>? self,
        IPacket packet
    )
        where E : Enum
        where T : class, IDynamicValue<E, T, F>
    ...
    )
    {
        ...
        for (int i = 0; i < count; i++)
            self.Add(read(packet));
    }

    internal static void ReadData<T>(this ICollection<T> self, IPacket packet)
        where T : IWireInterface, new() => self.ReadData(packet, ReadIntoNew<T>);

    internal static void WriteData<T>(
        this IReadOnlyCollection<T> self,
        IPacket packet,
        Action<IPacket, T> write
    )
    {
    ...
    internal static void WriteData<T>(this IReadOnlyCollection<T> self, IPacket packet)
        where T : IWireInterface => self.WriteData(packet, Write<T>);

    internal static void ReadData<T>(
        this HashMultiset<T> self,
        IPacket packet,
        Func<IPacket, T> read
    )
    {
    ...
    internal static void ReadData<T>(this HashMultiset<T> self, IPacket packet)
        where T : IWireInterface, new() => self.ReadData(packet, ReadIntoNew<T>);
    ...
    internal static void ReadData<K, V>(
        this IDictionary<K, V> self,
        IPacket packet,
        Func<IPacket, K> readKey,
        Func<IPacket, V> readValue
    )
    {
        self.Clear();
    ...
    internal static void ReadData<K, V>(
        ...
        where V : IWireInterface, new() => self.ReadData(packet, readKey, ReadIntoNew<V>);

    internal static void WriteData<K, V>(
        this IReadOnlyDictionary<K, V> self,
        IPacket packet,
        Action<IPacket, K> writeKey,
        Action<IPacket, V> writeValue
    )
    {
        packet.WriteVarint(self.Count);
    ...
    internal static void WriteData<K, V>(
        ...
        where V : IWireInterface => self.WriteData(packet, writeKey, Write<V>);

    internal static void ReadData<K, V>(
        this ListMultimap<K, V> self,
        IPacket packet,
        Func<IPacket, K> readKey,
        Func<IPacket, V> readValue
    )
    ...
    internal static void WriteData<K, V>(
        ...
        where V : IWireInterface, new() => self.WriteData(packet, writeKey, Write<V>);
}
