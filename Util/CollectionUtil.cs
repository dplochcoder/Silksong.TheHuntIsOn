...
internal static class CollectionUtil
{
        ...
        where V : struct
    {
        foreach (var key in left.Keys.Concat(right.Keys).Distinct())
        {
            callback(
                key,
                left.TryGetValue(key, out var leftValue) ? leftValue : null,
                right.TryGetValue(key, out var rightValue) ? rightValue : null
            );
        }
    }
