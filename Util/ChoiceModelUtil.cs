...
{
    internal static IntRange IntRange(int minInclusive, int maxInclusive) =>
        new(minInclusive, maxInclusive);

    internal static ListChoiceModel<int> IntRangeModel(int minInclusive, int maxInclusive) =>
        ChoiceModels.ForValues([.. IntRange(minInclusive, maxInclusive)]);
    ...
    internal static ListChoiceModel<int> FormatIntDelta(
        this ListChoiceModel<int> self,
        int defaultValue
    )
    {
...
internal class IntRange(int minInclusive, int maxInclusive) : IReadOnlyList<int>
{
    public int this[int index] =>
        (index >= 0 && index < Count)
            ? minInclusive + index
            : throw new IndexOutOfRangeException($"{index}; Count: {Count}");
